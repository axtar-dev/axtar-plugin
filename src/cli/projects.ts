...
import type { ProjectSummary } from '../shared/wire/schemas.js';
...
function explainFailure<T>(result: Extract<EngineResult<T>, { ok: false }>): string {
...
async function fetchProjects(client: EngineClient): Promise<ProjectSummary[]> {
...
function renderTable(projects: readonly ProjectSummary[]): void {
...
function resolveRef(
  ...
  if (byName.length === 1) {
    return { project: byName[0] as ProjectSummary };
  }
  if (byName.length > 1) {
    return {
...
async function cmdList(client: EngineClient, json: boolean): Promise<number> {
...
async function cmdSelect(client: EngineClient, ref: string | undefined): Promise<number> {
  if (!ref) {
...
async function cmdDoctor(client: EngineClient): Promise<number> {
