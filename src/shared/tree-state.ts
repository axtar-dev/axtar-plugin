...
/** The current state of a work tree, as the reminder reads it. */
export interface WorkTreeState {
  /** The fingerprint of this exact tree state. */
  hash: string;
  /** Whether git reports anything at all changed, staged or untracked. */
  dirty: boolean;
}

/** What is remembered about one repository, between turns. */
export interface ReminderState {
  /** The tree state a successful diff/scan check was run against. */
  last_checked_hash: string | null;
  /** The tree state the reminder has already fired for, once. */
  last_nudged_hash: string | null;
  /** When this file was last written, for a human reading it. */
  updated_at: string | null;
}

/** Nothing known yet — the answer for a repo with no state file, and for a corrupt one. */
export const EMPTY_STATE: ReminderState = {
 ...
 */
export async function readWorkTreeState(repoRoot: string): Promise<WorkTreeState | null> {
...
export async function workTreeStateHash(repoRoot: string): Promise<string | null> {
 ...
 */
export function stateFilePath(repoRoot: string, stateDir: string = defaultStateDir()): string {
...
export function readState(repoRoot: string, stateDir: string = defaultStateDir()): ReminderState {
...
export function writeState(
  repoRoot: string,
  patch: Partial<Omit<ReminderState, 'updated_at'>>,
 ...
 */
export async function markWorkTreeChecked(
  repoRoot: string,
  stateDir: string = defaultStateDir(),
): Promise<void> {
