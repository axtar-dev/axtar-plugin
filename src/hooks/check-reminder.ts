...
import type { ProducerOutcome } from '../shared/producer.js';
import { log } from '../shared/log.js';
import { loadRepoBinding } from '../shared/project/config.js';
import type { BindingResult } from '../shared/project/config.js';
import { defaultStateDir, readState, readWorkTreeState, writeState } from '../shared/tree-state.js';
import type { WorkTreeState } from '../shared/tree-state.js';
...
/**
 * The `Stop` payload, as Claude Code sends it. Every field is optional here on
 * purpose: a host that adds, renames or omits one must not turn into a crash at
 * the end of somebody's turn, and every missing field lands on the allow path.
 */
export interface StopHookInput {
  ...
  /** Set by Claude Code when a Stop hook already continued this turn. */
  stop_hook_active?: boolean | undefined;
}

/** Why the turn was allowed to end — stderr diagnostics, never shown to the agent. */
...
export type ReminderDecision =
  | { kind: 'allow'; reason: AllowReason }
  /** The one nudge: `reason` is what Claude Code shows the agent. */
  | { kind: 'nudge'; reason: string };

export interface ReminderDeps {
  /** Environment the off switch is read from. */
  env: Record<string, string | undefined>;
  /** Where to look for the repo when the payload names no `cwd`. */
  cwd: string;
  /** Where the per-repo state file lives — outside every repository. */
  stateDir: string;
  /** Seam for tests: work-tree root discovery. */
  findRoot: (cwd: string) => Promise<ProducerOutcome<string>>;
  /** Seam for tests: the tree fingerprint (shared with the MCP server). */
  readTree: (repoRoot: string) => Promise<WorkTreeState | null>;
  /** Seam for tests: the `.axtar/config.yml` binding. */
  loadBinding: (startDir: string) => BindingResult;
}

export function defaultDeps(): ReminderDeps {
  return {
    env: process.env,
    cwd: process.cwd(),
 ...
 */
export async function decide(input: StopHookInput, deps: ReminderDeps): Promise<ReminderDecision> {
  const off = deps.env[OFF_SWITCH_ENV];
  if (off !== undefined && off.trim().length > 0) return { kind: 'allow', reason: 'off_switch' };
  ...
  if (input.stop_hook_active === true) return { kind: 'allow', reason: 'stop_hook_active' };

  const start = input.cwd !== undefined && input.cwd.trim().length > 0 ? input.cwd : deps.cwd;

  const root = await deps.findRoot(start);
  if (!root.ok) return { kind: 'allow', reason: 'not_a_repo' };
  ...
  if (!deps.loadBinding(repoRoot).ok) return { kind: 'allow', reason: 'not_bound' };

  const tree = await deps.readTree(repoRoot);
  if (tree === null) return { kind: 'allow', reason: 'tree_unreadable' };
  if (!tree.dirty) return { kind: 'allow', reason: 'tree_clean' };
...
/** Claude Code's `Stop` output shape: `block` keeps the turn going, `reason` says why. */
 ...
 */
export function parseInput(raw: string): StopHookInput {
  try {
