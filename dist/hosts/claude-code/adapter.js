 ...
 */
export function renderVerdict(response, options) {
    ...
    const exitCode = (options.hook === 'PreToolUse' && verdict === 'block') || options.hook === 'PostToolUse'
        ? 2
        : 0;
    return {
        exitCode,
...
function formatStderr(verdict, violations, hook, ruleNames) {
    const lines = [];
...
function headerLine(verdict, hook) {
    const label = headerLabel(verdict, hook);
    return openBoxLine(label);
}
function headerLabel(verdict, hook) {
    if (hook === 'PreToolUse' && verdict === 'block')
        return 'Axtar blocked this edit';
    if (hook === 'PostToolUse' && verdict === 'block')
        return 'Axtar (post-edit): blocking rule violated';
    if (verdict === 'warn')
        return 'Axtar advisory';
    return 'Axtar';
}
/**
 ...
 */
function openBoxLine(label) {
    const head = `─── ${label} `;
    const fill = Math.max(3, BOX_WIDTH - head.length);
    return head + '─'.repeat(fill);
}
function appendViolation(lines, v, ruleNames) {
    ...
    if (spec || authored || since) {
        lines.push('');
        if (spec)
            lines.push(`${BODY_INDENT}Spec: ${spec}`);
        if (authored && since) {
...
function appendIndented(lines, text, indent) {
    for (const line of text.split('\n')) {
        lines.push(line.length > 0 ? `${indent}${line}` : '');
    }
}
function footerLine(verdict, hook) {
    if (hook === 'PreToolUse' && verdict === 'block')
        return 'Resolve and retry.';
    if (hook === 'PostToolUse' && verdict === 'block') {
        return 'Address the violation(s) above before continuing.';
    }
    return 'Note this and continue.';
}
export function renderEngineUnreachable(detail) {
...
// introduced).
// ───────────────────────────────────────────────────────────────────────────
export const claudeCodeOutputAdapter = {
 ...
 */
function boxMentor(label, message) {
    const lines = [];
