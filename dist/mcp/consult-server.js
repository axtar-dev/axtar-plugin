...
import { loadConsultConfig } from '../shared/engine/config.js';
...
export const CONSULT_INPUT_SHAPE = {
    session_id: z
        .string()
        .min(1)
        .describe('The session_id from the Mentor block message — relay it verbatim; do not invent one.'),
    files: z
        .array(z.string())
        .min(1)
        .describe('The file(s) this consultation covers (the flagged file). The server passes these through; do not pre-filter.'),
    question: z
        .string()
        .optional()
        .describe('Free-form question, e.g. "which layer should this live in?" or "is there precedent for X?"'),
    proposed_edit: z.string().optional(),
    plan: z.string().optional(),
    file_context: z.string().optional(),
};
export async function handleConsult(client, args, projectId) {
    ...
    const req = { session_id: args.session_id, files: args.files };
    if (args.question !== undefined)
        req.question = args.question;
    if (args.proposed_edit !== undefined)
        req.proposed_edit = args.proposed_edit;
    if (args.plan !== undefined)
        req.plan = args.plan;
    if (args.file_context !== undefined)
        req.file_context = args.file_context;
...
export const SESSION_SUMMARY_INPUT_SHAPE = {
    session_id: z
        .string()
        .min(1)
        .describe('The session_id whose summary to fetch — the same id the Mentor block surfaced. Relay it verbatim.'),
};
export async function handleSessionSummary(client, args) {
 ...
 */
export function consultEngineConfig() {
...
export async function main() {
    ...
    server.registerTool('consult', {
        ...
        description: 'Consult Axtar Mentor before a high-altitude edit. When the Mentor gate blocks an edit, call this with the session_id from the block message, the flagged file(s), and your question / proposed_edit / plan. An "approve" verdict clears the gate so the edit can proceed; "revise"/"block" do not.',
...
//# sourceMappingURL=consult-server.js.map