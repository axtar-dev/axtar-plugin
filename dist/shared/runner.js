...
function exitWithTrace(code, reason) {
    trace('runner.exit', { code, reason });
    process.exit(code);
}
export async function run(stdinRaw, options) {
    ...
    const filePathHint = typeof input.tool_input === 'object' && input.tool_input !== null
    ...
    const result = await client.evaluate(projectId ? { ...outcome.request, project_id: projectId } : outcome.request);
    if (!result.ok) {
    ...
    if (options.hook === 'PreToolUse' && result.value.consult_required) {
        ...
        const decision = decideGate(result.value, gateRes, {
            session_id: input.session_id,
            file_path: outcome.request.file_path,
            rule_set: outcome.request.rule_set,
        }, rung);
    ...
    const drift = options.hook === 'PostToolUse'
    ...
    if (drift !== null) {
        ...
        log.info('mentor drift advisory (v1 rule-scoped reminder); substance-drift comparison deferred to v2', { file_path: outcome.request.file_path });
    }
